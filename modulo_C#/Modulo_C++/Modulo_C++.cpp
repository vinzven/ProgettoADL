...
#include <vector>
#include <algorithm>
#include "include/json.hpp" // Include la libreria JSON per c++

using json = nlohmann::json;

// --- FUNZIONI DI AIUTO PER PULIRE I DATI ---

// Trasforma "4,00" in 4.00 (numero reale)
...
// Estrae il numero da " 2 giorni lavorativi" -> restituisce 2
int estrai_tempo(const std::string& tempo_str) {
    ...
    try {
		...
		return 999; // In caso di errore, sposta l'elemento in fondo alla lista
    }
}
...
int main() {
    ...
    if (std::getline(std::cin, stringa_json_da_python)) {
        try {
            //. Leggiamo l'array originale di offerte
            ...
            //  ORDINAMENTO PER PREZZO (Crescente: dal più basso al più alto)
            ...
            //  ORDINAMENTO PER TEMPO (Crescente: dal più veloce al più lento)
            ...
            // STAMPA IL RISULTATO PER MANDARLO A C# (tramite Python)
